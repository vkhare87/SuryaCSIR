...
description: Drafts new Supabase migration files for SURYA. Reads the 8-file domain baseline and existing schema context, then writes a correctly-ordered timestamped migration file with RLS enabled by default.
---
...
1. **Never edit** a shipped file in `supabase/migrations/` (the 8-file baseline, `20260712000001`–`20260712000008`). New work goes in a new timestamped file after it: `supabase/migrations/YYYYMMDDHHMMSS_description.sql`.
...
1. Read the relevant baseline file(s) in `supabase/migrations/` (`20260712000001`–`20260712000008`, see `supabase/ops/README.md` for the stage table) for context on existing tables and helpers.
